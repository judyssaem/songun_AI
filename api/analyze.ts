...
export default async function handler(req: any, res: any) {
  // Support CORS if needed, and handle preflight OPTIONS request
...
응답은 반드시 정해진 JSON 스키마만 출력하세요.`;
    ...
    return res.status(200).json({ success: true, result: resultJson });

  } catch (error: any) {
    console.error("Gemini 분석 오류 (Vercel):", error);
